...
@author: manwang
'''
from PyQt4.QtGui import QTransform, QGraphicsItem, QGraphicsEllipseItem, QBrush, QFont, QPen, QColor, QFontMetrics, QPainterPath, QGraphicsTextItem
from PyQt4.QtCore import Qt, QRectF, QObject, QSize, QRect, QPoint
import os, math
...
class FileNodeTextItem(QGraphicsTextItem):
    def __init__(self, name, parent, main):
        QGraphicsTextItem.__init__(self, name, parent)
        self.parent = parent
#         self.setFont(main.sysFont)
        setFontForUI(self, 19)
        self.setAcceptsHoverEvents(True)
        
    def hoverEnterEvent(self, evt):
        self.setToolTip("<font size=\"5\">%s</font>"%self.parent.getFullPath())
        QGraphicsTextItem.hoverEnterEvent(self, evt)
        
    def hoverLeaveEvent(self, evt):
        QGraphicsTextItem.hoverLeaveEvent(self, evt)
...
class FileNode(QGraphicsEllipseItem):
    ...
    def __init__(self, dirpath, name, isFile, main = None, specNode = False):
        ...
        self.userAccessibleColor = Qt.magenta#0
...
#             idExtenstion = self.name.rfind('.')
#             if self.isFile and idExtenstion == -1:
#                 self.displayName += self.name[idExtenstion+1:]
        return displayName
...
#         painter.setFont(self.main.sysFont)
        ...
        name = self.displayName#self.getDisplayName()#
#         if self.drawCross:
#             self.setBrush(Qt.lightGray)
#             pen = QPen(Qt.gray)
#             self.setPen(pen)
        QGraphicsEllipseItem.paint(self, painter, option, widget)
#         if self.accessible == 3:
#             pen = QPen(self.permAccessibleColor)
#             pen.setWidth(2.0) 
#             self.setPen(pen)
        if self.parent:
            ...
            rect.moveTo(x, y)
            painter.drawText(rect, Qt.AlignCenter, 'Root')
